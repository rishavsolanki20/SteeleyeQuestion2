function highlightHTMLContent(htmlContent, plainText, plainTextPositions) {
    ...
    for (let i = plainTextPositions.length - 1; i >= 0; i--) {
        ...
        let indexInHtml = result.indexOf(textToHighlight);
        if (indexInHtml !== -1) {
            result = result.substring(0, indexInHtml) + '<mark>' + textToHighlight + '</mark>' + result.substring(indexInHtml + textToHighlight.length);
        }
    }
    return result;
}
